...
export function renderAcordeon(sorteos, uiState) {
  ...
  for (const s of sorteos) {
    ...
    // Pendientes primero, entregadas después
    ...
    head.innerHTML = `
      ...
      <div class="ac-badges">
        ...
        <span class="badge neutral">Mostrando: ${filtrados.length}</span>
      </div>
    ...
    // Render: pendientes primero y luego entregadas (pero ya filtrados)
    for (const p of filtrados) {
      ...
      row.innerHTML = `
        ...
        <div class="user-right">
          ...
          <a class="btn-mini" ${p.email ? `href="mailto:${p.email}?subject=${encodeURIComponent('Siempre Gana — Entrega de cuenta')}"` : ''}
  ...
  // Si tras filtros no quedó nada visible
